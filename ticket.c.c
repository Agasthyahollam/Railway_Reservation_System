...
fprintf(fprintf ,"Train name: %s\t\t\t\t Train number: %s\n",trname,r_no_choice);
printf(fprintf,"Origin: %s \t\t\t Destination :%s \n" ,origin_station,destination);
printf(fprintf,"Arrival at Origin: %s \t Departure from Origin: %s \t Arrival at Destination:%s \n",arr_origin,dep_dest,arr_dest);
printf(fprintf,"No of Seats Booked: %d",seat_num);
printf(fprintf,"\t\t\t\t\tTier Booked : %s\n",class_choice);
...
fprintf(fptr1,"Payment Method:%s\n",choice1);
